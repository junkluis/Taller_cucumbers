...
@given("un conjunto de películas con idioma")
...
@given("el usuario ingresa el idioma: '{rating}'")
def step_impl(context, rating):
	context.rating = rating


@when("busque la películas por idioma {criterio}")
def step_impl(context, criterio):
	if(criterio == 'rating'):
		resultado, mensaje = get_pelicula_idiomas(context.peliculas, context.rating)
		print(resultado)
		context.resultado = resultado
		context.mensaje = mensaje


@then("obtendrá {total} películas que coincidan con el idioma")
...
@then("obtiene el siguiente mensaje2 '{mensaje}'")
