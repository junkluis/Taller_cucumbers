...
def step_impl(context):
    ...
    for row in context.table:
        ...
        idiomas = []
        pelicula = Pelicula(
            row['TITULO'], row['DIRECTOR'],
            row['ELENCO'].split(","), row['GENERO'],
            row['IDIOMAS'].split(","), int(row['ESTRENO']),
            row['RAITING'])
...
@given("el usuario ingresa los ratings: '{ratings}'")
...
@given("el usuario ingresa las fechas: '{fecha_inicial}' y '{fecha_final}'")
def step_impl(context, fecha_inicial, fecha_final):
    context.fecha_inicial = int(fecha_inicial)
    context.fecha_final = int(fecha_final)


@given("el usuario no ingresa fechas")
def step_impl(context):
    context.fecha_inicial = None
    context.fecha_final = None


@when("busque la películas por {criterio}")
def step_impl(context, criterio):
    if(criterio == 'título'):
        resultado, mensaje = get_pelicula_titulo(
            context.peliculas, context.titulo)
        print(resultado)
        context.resultado = resultado
        context.mensaje = mensaje
    elif(criterio == 'ratings'):
        resultado, mensaje, error = get_pelicula_rating(
            context.peliculas, context.ratings)
        print(resultado)
        context.resultado = resultado
        context.mensaje = mensaje
        context.error = error
    elif(criterio == 'idioma'):
        resultado, mensaje = get_pelicula_idiomas(
            context.peliculas, context.idioma)
        print(resultado)
        context.resultado = resultado
        context.mensaje = mensaje
    elif(criterio == 'fecha'):
        if(context.fecha_inicial is None and context.fecha_final is None):
            resultado, mensaje = get_pelicula_fecha_estreno(context.peliculas)
        else:
            resultado, mensaje = get_pelicula_fecha_estreno(
                context.peliculas, context.fecha_inicial, context.fecha_final)
        context.resultado = resultado
        context.mensaje = mensaje
